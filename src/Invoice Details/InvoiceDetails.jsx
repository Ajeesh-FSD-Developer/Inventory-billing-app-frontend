...
export default function InvoiceDetails() {
  ...
  useEffect(() => {
    const fetching = async () => {
      ...
      const res = await fetch(
        `https://inventory-billing-app-backend-roes.onrender.com/api/invoices/getallInvoices/${currentUser._id}?${params.toString()}`,{
  ...
  const handledelete = async (deleteid) => {
    try {
      const t =localStorage.getItem("access_token");
      const res = await fetch(
        `https://inventory-billing-app-backend-roes.onrender.com/api/invoices/deleteinvoice?deleteid=${deleteid}`,
        {
          method: "DELETE",
          headers:{
            Authorization: `Bearer ${t}`,
