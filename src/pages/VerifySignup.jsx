...
export default function VerifySignup() {
  ...
  useEffect(() => {
    const fetching = async () => {
      try {
        const res = await fetch(`https://inventory-billing-app-backend-roes.onrender.com/api/admin/verify/${id}/${token}`);
  ...
  const handlesubmit = async () => {
    const res = await fetch(
      `https://inventory-billing-app-backend-roes.onrender.com/api/admin/verificationstatus/${id}/${token}`
    );
    const data = await res.json();
    if (data.success === false) {
      setError(data.message);
